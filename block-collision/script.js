...
function draw() {
    ...
    // Stop the simulation and time increment after Block B hits the edge
    if (blockB.x + blockB.width > canvas.width) {
        blockB.x = canvas.width - blockB.width; // Make sure it doesn't go off-screen
        return; // Stop the animation loop (don't call requestAnimationFrame anymore)
    }
